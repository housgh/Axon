...
public class AxonSQLiteDeviceConnectionStoreTests(SQLiteFixture fixture)
{
    // GetConnectionId/GetAll join against ServerInstances and filter by heartbeat freshness, and
    // AxonSQLiteDeviceConnectionStore.Register always stamps rows with the current process's
    // static AxonServerInstanceHeartbeat.InstanceId - so every row in these tests is "owned" by
    // that one instance, and we heartbeat/age it directly via AxonSQLiteInstanceStore to
    // control whether it reads as online or stale. Device names and connection ids are
    // randomized per test since tests in this class run concurrently against the same database.
    private readonly string _instanceId = AxonServerInstanceHeartbeat.InstanceId;

    private AxonSQLiteDeviceConnectionStore CreateSut() => new(fixture.ConnectionString);
    private AxonSQLiteInstanceStore CreateInstanceStore() => new(fixture.ConnectionString);
