...
public class AxonSqlServerDeviceConnectionStoreTests(SqlServerFixture fixture)
{
    // GetConnectionId/GetAll join against ServerInstances and filter by heartbeat freshness, and
    // AxonSqlServerDeviceConnectionStore.Register always stamps rows with the current process's
    // static AxonServerInstanceHeartbeat.InstanceId - so every row in these tests is "owned" by
    // that one instance, and we heartbeat/age it directly via AxonSqlServerInstanceStore to
    ...
    private AxonSqlServerDeviceConnectionStore CreateSut() => new(fixture.ConnectionString);
    private AxonSqlServerInstanceStore CreateInstanceStore() => new(fixture.ConnectionString);
