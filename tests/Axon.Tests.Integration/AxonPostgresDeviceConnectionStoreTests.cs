...
public class AxonPostgresDeviceConnectionStoreTests(PostgresFixture fixture)
{
    // GetConnectionId/GetAll join against ServerInstances and filter by heartbeat freshness, and
    // AxonPostgresDeviceConnectionStore.Register always stamps rows with the current process's
    // static AxonServerInstanceHeartbeat.InstanceId - so every row in these tests is "owned" by
    // that one instance, and we heartbeat/age it directly via AxonPostgresInstanceStore to
    ...
    private AxonPostgresDeviceConnectionStore CreateSut() => new(fixture.ConnectionString);
    private AxonPostgresInstanceStore CreateInstanceStore() => new(fixture.ConnectionString);
