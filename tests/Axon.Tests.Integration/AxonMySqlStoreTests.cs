...
public class AxonMySqlStoreTests(MySqlFixture fixture)
{
    private AxonMySqlStore CreateSut() => new(fixture.ConnectionString);
    ...
    [Fact]
    public async Task AddJob_WithRetryPolicy_RoundTripsThroughMySql()
    {
        var sut = CreateSut();
    ...
    [Fact]
    public async Task AddJob_WithContinuationFields_RoundTripsThroughMySql()
    {
        var sut = CreateSut();
    ...
    [Fact]
    public async Task TryClaimJob_ConcurrentCallersAgainstRealMySql_ExactlyOneWins()
    {
        // This is the core distributed-dispatch safety guarantee: simulates multiple
        // Axon.Server instances racing to claim the same due job via the shared MySQL store.
        var sut = CreateSut();
        var job = JobFactory.CreateJob(state: JobState.Enqueued);
        await sut.AddJob(job);
    ...
    [Fact]
    public async Task TryClaimJob_ConcurrencyLimitNotReached_ClaimsAgainstRealMySql()
    {
    ...
    [Fact]
    public async Task TryClaimJob_ConcurrencyLimitReached_RejectsAgainstRealMySql()
    {
        var concurrencyKey = Guid.NewGuid().ToString();
        var sut = CreateSut();
    ...
    [Fact]
    public async Task TryClaimJob_ConcurrencyLimitedJobs_ConcurrentCallersAgainstRealMySql_NeverExceedLimit()
    {
        // The core correctness claim for this feature: racing 10 real claims against real
        // MySQL, all sharing a concurrency key with MaxConcurrent=3, must never let more than 3
        // end up Processing - proving the FOR UPDATE count-then-update in
        // AxonMySqlStore.TryClaimJob actually serializes the count-check against real
    ...
    public async Task DeleteCompletedJobsOlderThan_NeverDeletesNonTerminalJobsAgainstRealMySql(JobState nonTerminalState)
    {
        var sut = CreateSut();
