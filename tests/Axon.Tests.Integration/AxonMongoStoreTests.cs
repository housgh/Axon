...
public class AxonMongoStoreTests(MongoDbFixture fixture)
{
    private AxonMongoStore CreateSut() => new(fixture.ConnectionString, fixture.DatabaseName);
    ...
    public async Task AddJob_ThenGetJob_RoundTripsAllFields()
    {
        ...
        // Arguments round-trip through JSON (see AxonMongoStore.ToDocument), so they come back
        // as JsonElement rather than the original CLR types - compare via JSON representation.
    ...
    [Fact]
    public async Task AddJob_WithRetryPolicy_RoundTripsThroughMongo()
    {
        var sut = CreateSut();
    ...
    [Fact]
    public async Task AddJob_WithContinuationFields_RoundTripsThroughMongo()
    {
        var sut = CreateSut();
    ...
    [Fact]
    public async Task TryClaimJob_ConcurrentCallersAgainstRealMongo_ExactlyOneWins()
    {
        // This is the core distributed-dispatch safety guarantee: simulates multiple
        // Axon.Server instances racing to claim the same due job via the shared Mongo store.
        var sut = CreateSut();
        var job = JobFactory.CreateJob(state: JobState.Enqueued);
        await sut.AddJob(job);
    ...
    [Fact]
    public async Task TryClaimJob_ConcurrencyLimitNotReached_ClaimsAgainstRealMongo()
    {
        // ConcurrencyKey must be unique per test: the limit is a count over every document
        // sharing the key, in a database shared across all tests in this fixture, so a fixed
        // literal key (unlike JobId, which is already a fresh GUID per test) would let one
        // test's leftover Processing documents pollute another test's count.
    ...
    [Fact]
    public async Task TryClaimJob_ConcurrencyLimitReached_RejectsAgainstRealMongo()
    {
        var concurrencyKey = Guid.NewGuid().ToString();
        var sut = CreateSut();
    ...
    [Fact]
    public async Task TryClaimJob_ConcurrencyLimitedJobs_ConcurrentCallersAgainstRealMongo_NeverExceedLimit()
    {
        // The core correctness claim for this feature: racing 10 real claims against real
        // MongoDB, all sharing a concurrency key with MaxConcurrent=3, must never let more than
        // 3 end up Processing - proving the transaction-scoped count-then-update in
        // AxonMongoStore.TryClaimJob actually serializes the count-check against real
        // concurrent transactions (via write-conflict retry), not just against
        // sequential/mocked calls.
        var concurrencyKey = Guid.NewGuid().ToString();
        var sut = CreateSut();
    ...
    [Fact]
    public async Task DeleteCompletedJobsOlderThan_NeverDeletesFailedOrSkippedJobsAgainstRealMongo()
    {
        var sut = CreateSut();
    ...
    [Fact]
    public async Task CountJobsByState_CountsSoftDeletedSucceededJobAgainstRealMongo()
    {
    ...
    public async Task DeleteCompletedJobsOlderThan_NeverDeletesNonTerminalJobsAgainstRealMongo(JobState nonTerminalState)
    {
        var sut = CreateSut();
