...
public class AxonSqlServerStore(string connectionString) : IAxonJobStore
{
    ...
    public Task<Dictionary<JobState, int>> CountJobsByState() => SqlExceptionTranslator.Run(connectionString, async () =>
    {
        await using var conn = CreateConnection();
        ...
        const string sql = @"
            ...
            WHERE IsDeleted = 0 OR State IN (@Succeeded, @Failed, @Skipped)
            GROUP BY State";
